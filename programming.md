...
{: .warning}
**This document describes the upstream `md-microfirmware-template` framework, not the trimmed `md-framebuffer-template` you are looking at.** Many modules referenced below (`chandler.c`, `tprotocol.c`, `display.c`, `term.c`, `download.c`, the network stack, etc.) have been stripped or replaced in this template. Refer to `CLAUDE.md` for the **current** module list and architecture, and `docs/epics/` for the per-epic rationale of each removal. The most recent relevant change is **Epic 3 Story 3.8** (chandler + TPROTOCOL command dispatch dropped; cart-region constants relocated to `rp/src/include/cart_shared.h` under a `CART_*` prefix; the main loop now drains commemul directly via `commemul_poll(ikbd_consume_rom3_sample)`).
