...
1. **RP draws into cart RAM** at `$FA8300` (mirrored at RP `0x20030000 + 0x8300`). `fb.c` owns `fb_screen.framebuffer`; `fb_font.c` / `fb_draw.c` provide text + pixel primitives.
2. **m68k blits cart FB → ST screen page** inside `userfw.s`'s VBL loop. Two paths, picked once per boot by walking the `_MCH` cookie:
...
3. **m68k flips video base** to the page it just wrote. `userfw.s` toggles `A4` between `$70000` and `$78000` every frame, so the display always reads from a stable page while the next blit fills the other.
4. **PALETTE_IDX0 (`$FFFF8240`) doubles as a timing tape-measure** — `userfw.s` writes it at three points (post-vsync, blit-start, blit-end) so the ST border shows the blit cost as colored bands. White band = blit in flight (≈4 ms STE, ≈10 ms ST). Green/blue band = slack until next vsync.
...
- `fb.c` / `fb.h` — **owns the 32 KB framebuffer at `$FA8300`.** `fb_init(&fb_mode_320x200)` populates `fb_screen` (pointer + width + height + bpp), builds `pixel_masks_flat`, and clears the FB to 0xFF (= palette index 15 = black on default TOS palette). `fb_render_static()` draws the parts that never change (called once at boot); `fb_render_frame()` redraws only the rows that animate (called every main-loop iteration). Surgical redraws keep the per-frame RP write small (~2.5 KB) so the m68k blit rarely catches mid-write.
- `fb_font.c` / `fb_draw.c` — text + pixel primitives ported from md-sprites-demo's `vga_font.c` / `vga_draw.c`. `font_set_font(&font6x8)`, `font_set_color(0)` for white text on black, `font_move(x, y)`, `font_print(str)` / `font_printf(fmt, ...)`. Pixel writes use a 64-bit LUT (`pixel_masks_flat`) targeting aligned slots — the cart-bus byte-swap is invisible at the word level, so no swap-awareness is needed in the font path (only byte-level pokes via `fb_plot` would need it, and the template no longer ships one).
