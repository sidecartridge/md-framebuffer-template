...
- `emul.c` / `emul.h` — the application's main loop and entry point. ~100 lines: erase + copy firmware to RAM, init IKBD ring, init romemul, init fb, init commemul, mount SD, configure SELECT, bring up the demo dispatcher (`demo_dispatcher_init()`), then loop `{commemul_poll(ikbd_consume_rom3_sample); ikbd_pump(); drain key events → demo_dispatcher_handle_key(); demo_dispatcher_render_frame(); audio_render_frame();}`. The demo dispatcher (`demo_menu.c`) is the per-frame render driver; apps add foreground work inside that loop.
