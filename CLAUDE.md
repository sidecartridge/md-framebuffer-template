...
2. Builds the Atari ST target (`target/atarist/build.sh`) via `stcmd make`. `gen_fbdrv.py` emits `target/atarist/src/fbdrv.s` (the MOVEM-loop cart→ST screen copy) before vasm runs. Enforces a **16 KB hard limit** on `BOOT.BIN` (the cartridge code budget — `CART_CARTRIDGE_CODE_SIZE` in `rp/src/include/cart_shared.h`, mirrored as `CARTRIDGE_CODE_SIZE` in `target/atarist/src/main.s`); a build that exceeds it aborts with `ERROR: cartridge code is N bytes; limit is 16384`. A separate copy (`FIRMWARE.IMG`) is then padded to 64 KB to fill the entire shared region, and `firmware.py` converts it into `rp/src/include/target_firmware.h` (a C byte array embedded in the RP firmware).
